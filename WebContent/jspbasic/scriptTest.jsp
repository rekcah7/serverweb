...
<body>
	...
	<h4>==> 표현식으로 정의하는 데이터는 out.print()의 매개변수로 전달 도니다.
		따라서 표현식으로 정의할때는 ;을 추가하지 않는다. ;을 추가하는 것은 print(값;)와 동일한 결과가
		된다.
		표현식은 자바에서 실행되는 값을 웹페이지에 출력할 목적으로 사용하는 jsp스크립트 요소이며
		사용가능한 타입은 제한적이다. 타입이 제한적인 이유는 서블릿으로 변환될때 print메소드의 매개변수로
		전달되므로...기본형, 스트링으로 변환될 수 있는 변수의 값, 메소드의 호출결과 연산식 등을 사용할 수 있다.
	</h4>
	...
	<h3>1. 주석문</h3>
	<!-- html주석문(클라이언트로 전송) -->
	<%-- jsp주석문(jsp코드를 주석으로 처리하는 용도 : 클라이언트에 전송되지 않는다.) --%>
	<%
			//자바주석문
			/*
			자바 여러 줄 주석문
